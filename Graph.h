...
//struct containing adress destination node and the weight itself
struct Edge{
    string source;
    string adress;
    double weight;
...
//struct to determine a node in the graph
struct Node{
    string id;              // a unique identifier for each node
    vector<Edge> edges;     //set of all nodes that are connected
...
class DisjointSet{
    ...
    unordered_map<string, int> rank;      //rank of each node

    public:
    //initialize the default constructor
    DisjointSet();
    
    DisjointSet(const unordered_map<string, Node>& nodes);
...
class Graph {
    ...
    public:
        ...
        /**
         * Given a threshold, ignoring all edges with a weight greater than the threshold, return the connected components of 
         the resulting graph as a `vector` of `vector` of `string` (i.e., each connected component is a `vector` of `string`, 
         and you return a `vector` containing all of the connected components).
         * The components can be in any order, and the node labels within a component can be in any order.
         * Example: If our graph has edges "A"<-(0.1)->"B", "B"<-(0.2)->"C", "D"<-(0.3)->"E", and "E"<-(0.4)->"F", i
         f our threshold is 0.3, we would output the following connected components: {{"A","B","C"}, {"D","E"}, {"F"}}
         * @param threshold The maximum edge weight to consider
         * @return The connected components of this graph, if we ignore edges with weight greater than `threshold`, 
         as a `vector<vector<string>>`.
         */
